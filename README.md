...
The project contains both the fluid solver library source code written in C++ and an example usage of it to create an interactive visualization using the Unity game engine.

No additional libraries have been used so far for the development of the project besides [Catch2](https://github.com/catchorg/Catch2) for unit testing, with the main reason being that I want to implement the solvers and the data structures myself.

## Features
The simulation discretizes the velocity field on a MAC (Marker-And-Cell) grid. It also features semi-Lagrangian advection, and the Jacobi method for solving the pressure Poisson equation, although my intention is to experiment with other advection schemes that don't cause as much numerical diffusion as well as other faster converging linear solvers. For the time being there's two types of cells in the simulation: fluid or solid.
