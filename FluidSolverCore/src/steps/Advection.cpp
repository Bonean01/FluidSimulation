...
void Advection::execute(MACGrid2D& velocityField, const Grid2D<uint8_t>& solidCellMap, float timeStep) {
	...
	for (int j = 0; j < height + 1; j++) {
		for (int i = 0; i < width; i++) {
			if (solidCellMap.getValue(i, j) || solidCellMap.getValue(i, j - 1)) { m_auxMacGrid.setEdgeY(i, j, 0.0f); continue; }
...
void Advection::execute(ScalarField2D& field, const MACGrid2D& velocityField, const Grid2D<uint8_t>& solidCellMap, float timeStep) {
	...
	for (int j = 0; j < height; j++) {
		for (int i = 0; i < width; i++) {
			if (solidCellMap.getValue(i, j)) { m_auxScalarField.setValue(i, j, 0.0f); continue; }
